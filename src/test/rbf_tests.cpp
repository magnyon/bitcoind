...
BOOST_FIXTURE_TEST_CASE(rbf_conflicts_calculator, TestChain100Setup)
{
    ...
    // Create a parent transaction with 200 outputs
    const int NUM_OUTPUTS = 200;
    ...
    const auto parent_tx = make_tx(/*inputs=*/ {m_coinbase_txns[0]}, /*output_values=*/ output_values);
    pool.addUnchecked(entry.Fee(normal_fee).FromTx(parent_tx));

    std::vector<CTransactionRef> direct_children;

    // Create lots of spends of those outputs in separate chains.
    for (auto i = 0; i < NUM_OUTPUTS; ++i) {
        auto pretx = make_tx(/*inputs=*/ {parent_tx}, /*output_values=*/ {995 * CENT});
        CMutableTransaction tx(*pretx);
        tx.vin[0].prevout.n = i;
        pool.addUnchecked(entry.Fee(normal_fee).FromTx(tx));
        direct_children.push_back(MakeTransactionRef(tx));
        add_descendants(direct_children.back(), 10, pool);
    }
    ...
    // limit.
    const auto parent_entry = pool.GetIter(parent_tx->GetHash()).value();
    const auto conflicting_transaction = make_tx({parent_entry->GetSharedTx()}, {50 * CENT});
    CTxMemPool::setEntries all_conflicts, dummy;
    BOOST_CHECK(GetEntriesForConflicts(/*tx=*/ *conflicting_transaction.get(),
                                       /*pool=*/ pool,
                                       /*iters_conflicting=*/ {parent_entry},
